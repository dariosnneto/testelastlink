...
test.describe('Concurrency', () => {
  ...
  // ---------------------------------------------------------------------------
  test(
    'CT41 - concurrent creates with same Idempotency-Key all return same payment_id',
    { tag: ['@concurrency', '@idempotency', '@critical'] },
    async ({ playwright }) => {
      // Arrange
  ...
  // ---------------------------------------------------------------------------
  test(
    'CT42 - concurrent captures on the same payment produce only 200/422 and at least one 200',
    { tag: ['@concurrency', '@critical'] },
    async ({ playwright }) => {
  ...
  // ---------------------------------------------------------------------------
  test(
    'CT43 - concurrent creates without Idempotency-Key all return unique payment_ids',
    { tag: ['@concurrency'] },
    async ({ playwright }) => {
      // Arrange
      const payload = validPaymentPayload();
  ...
  // ---------------------------------------------------------------------------
  test(
    'CT44 - ledger has exactly 3 entries (no duplicates) after concurrent captures',
    { tag: ['@concurrency', '@ledger', '@critical'] },
    async ({ playwright }) => {
