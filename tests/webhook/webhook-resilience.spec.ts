...
test.describe('Webhook Resilience', () => {
  ...
  // afterEach provides a symmetric cleanup guarantee even when a test fails.
  // ---------------------------------------------------------------------------
  ...
  // ---------------------------------------------------------------------------
  test(
    'CT53 - capture fires webhook and returns 200 immediately (mode=ok)',
    { tag: ['@webhook', '@smoke'] },
    async ({ request }) => {
      // Arrange
  ...
  // ---------------------------------------------------------------------------
  test(
    'CT54 - capture returns 200 APPROVED even when webhook returns 500 (mode=500)',
    { tag: ['@webhook', '@resilience', '@critical'] },
    async ({ request }) => {
      // Arrange
  ...
  // ---------------------------------------------------------------------------
  test(
    'CT55 - capture returns quickly even when webhook sink delays 10 s (mode=timeout)',
    { tag: ['@webhook', '@resilience', '@critical'] },
    async ({ request }) => {
      // Arrange
  ...
  // wedge the server.
  //
  // NOTE: sleep() is unavoidable here — retry exhaustion has no observable
  // API signal, so polling cannot replace the fixed wait.
  // ---------------------------------------------------------------------------
  test(
    'CT56 - API is fully operational after all webhook retries are exhausted (mode=500)',
    { tag: ['@webhook', '@resilience'] },
    async ({ request }) => {
      test.slow(); // triples the project timeout (25 s × 3 = 75 s)
  ...
  // ---------------------------------------------------------------------------
  test(
    'CT57 - reject returns immediately in timeout mode (webhook is not called on reject)',
    { tag: ['@webhook', '@resilience'] },
    async ({ request }) => {
      // Arrange
      await setWebhookMode(request, 'timeout');
