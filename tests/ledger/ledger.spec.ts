...
test.describe('Ledger endpoint', () => {
  ...
  test('CT45 — captured payment returns 200 with payment_id and entries array', async ({ request }) => {
    // Arrange
  ...
  test('CT46 — entry count equals 1 debit plus one credit per split item', async ({ request }) => {
    // Arrange
    const split = [
  ...
  test('CT47 — debit entry has correct type, account, and full payment amount', async ({ request }) => {
    // Arrange
    const amount = 10000;
  ...
  test('CT48 — credit entries match recipients and computed amounts', async ({ request }) => {
    // Arrange
    const amount = 10000;
    const split = [
  ...
  test('CT49 — sum of credit amounts equals the debit amount', async ({ request }) => {
    // Arrange
    const { payment } = await createAndCapture(request);
  ...
  test('CT50 — unknown payment ID returns 404', async ({ request }) => {
    // Act
  ...
  test('CT51 — PENDING payment has no ledger entries (404)', async ({ request }) => {
    // Arrange
  ...
  test('CT52 — rejected payment has no ledger entries (404)', async ({ request }) => {
    // Arrange
